---
name: plan
description: Turn a bet pitch into an executable spec. Decompose into independent scopes with machine-checkable exit criteria, identify parallel-dispatch opportunities, initialize hill chart. Replaces /design + the back of /analyze.
---
...
2. **Define exit criteria per scope (strict rule)** — every scope ≥1 machine-checkable command. UI scopes mandatorily include `pnpm build:web` exit 0. AI scopes mandatorily include the golden eval case from pitch.
...
5. **Wireframes (UI scopes only, light)** — ASCII layout + Mermaid state diagram. No component-flow Mermaid; no pixel-perfect.
6. **Initialize hill chart** — all scopes at `uphill 0%`.
...
- Every UI scope includes `pnpm build:web` exit 0
